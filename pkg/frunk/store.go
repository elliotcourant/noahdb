...
// ExecuteEx executes queries that return no rows, but do modify the database.
...
// QueryEx executes queries that return rows, and do not modify the database.
...
func (s *Store) Apply(l *raft.Log) interface{} {
	...
	case kvSet:
		var d keyValueSetSub
		if err := json.Unmarshal(c.Sub, &d); err != nil {
			return &fsmGenericResponse{error: err}
		}
		for k, v := range d.Data {
			if err := s.boltStore.Set([]byte(k), []byte(v)); err != nil {
				return &fsmGenericResponse{error: err}
			}
		}
		return &fsmGenericResponse{}
	case kvDelete:
		var d keyValueDeleteSub
		if err := json.Unmarshal(c.Sub, &d); err != nil {
			return &fsmGenericResponse{error: err}
		}
		for _, k := range d.Keys {
			if err := s.boltStore.Delete([]byte(k)); err != nil {
				return &fsmGenericResponse{error: err}
			}
		}
		return &fsmGenericResponse{}
