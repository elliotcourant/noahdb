...
func TestNetworkTransport_CloseStreams(t *testing.T) {
	...
	trans1, err := NewTCPTransportWithLogger("127.0.0.1:0", nil, 2, time.Second, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	defer trans1.Close()
	...
	trans2, err := NewTCPTransportWithLogger("127.0.0.1:0", nil, 3, time.Second, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	defer trans2.Close()
...
func TestNetworkTransport_StartStop(t *testing.T) {
	trans, err := NewTCPTransportWithLogger("127.0.0.1:0", nil, 2, time.Second, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	trans.Close()
...
func TestNetworkTransport_Heartbeat_FastPath(t *testing.T) {
	...
	trans2, err := NewTCPTransportWithLogger("127.0.0.1:0", nil, 2, time.Second, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	defer trans2.Close()
...
func TestNetworkTransport_EncodeDecode_AddressProvider(t *testing.T) {
	...
	config := &NetworkTransportConfig{MaxPool: 2, Timeout: time.Second, Logger: nil, ServerAddressProvider: &testAddrProvider{addressOverride}}
...
func makeTransport(t *testing.T, useAddrProvider bool, addressOverride string) (*NetworkTransport, error) {
	...
	return NewTCPTransportWithLogger("127.0.0.1:0", nil, 2, time.Second, nil)
}
